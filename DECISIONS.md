...
最終更新: 2026-06-09（業販対応 段2の設計確定・実装は次回）
...
| 業販対応（法人/個人で明細の見せ方を変える） | 🚧 第一歩・第二歩-1・2-2a 本番反映済。段2は設計確定・実装これから | main | 次=段2-1（明細画面：個人=内容/数量/単価/小計、法人=内容/数量/単価/業販/参考定価＋合計欄に定価税込併記、定価をOrderItem保存）→段2-2（PDF）。設計詳細は意思決定ログ参照 |
