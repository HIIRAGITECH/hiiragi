...
最終更新: 2026-06-09（業販対応2-2a手元保持・メニュー単機能化の方向確定）
...
| 業販対応（法人/個人で明細の見せ方を変える） | 🚧 第一歩・第二歩-1 本番反映済。2-2a（明細への業販価格反映）は実装済みだが**手元保持・未デプロイ** | main（2-2aは未commit/未push） | 次は先に「メニュー単機能化」を片付けてから業販対応の続き（明細の見せ方切替・PDF）に戻る。意思決定ログ参照 |
| メニュー単機能化／部品在庫一本化 | 🔜 方向確定・実装これから | まだコードなし | 2026-06-09設計。部品は部品在庫に一本化、メニューは「部品以外の金額項目」単一化。本番移行不要（調査済）。意思決定ログ参照 |
...
- **2026-06-09** ｜ 業販対応 第二歩-2a（実装済・手元保持） ｜ 法人受注で部品/メニューを呼ぶと業販価格（定価×掛け率）が明細に入る計算ロジック。isBusinessをrowFromPart/rowFromMenuに引数で渡し分岐。PartPickerModalに法人時「業販¥」表示。OrderItem構造不変・計算式無改造。**メニュー単機能化で手直しが入るため、commitせず手元保持→単機能化と合わせて反映予定**。devブラウザ検証も未実施 ｜ コードのみ（未デプロイ）
- **2026-06-09** ｜ メニュー設計 方向確定 ｜ 「部品は部品在庫に一本化／メニューは部品以外の金額項目に単機能化／工賃概念を持たない」方向を確定（意思決定ログ参照）。本番移行不要を調査で確認。実装は次回、影響範囲（集計・PDF・既存データ）調査後 ｜ 記録のみ
- （以降追記）
