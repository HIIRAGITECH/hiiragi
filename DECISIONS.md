...
## 2.5. 次にやることリスト（TODO・状態つき）

> 最終更新: 2026-07-16。散在する「残課題」を1か所に集約した横断TODO。優先度・着手判断は各項目の状態欄に従う。
> 詳細な経緯は §3ロードマップ・§4意思決定ログ・§7アイデア帳を参照。

### 部品カテゴリ
- ✅ **段階1〜3 完了・本番反映済み**：テナント別・最大3階層のカテゴリ作成（段階1）／部品への紐付け＝カスケード選択（段階2）／一覧のカテゴリ絞り込み＝左ツリー＋右一覧・件数バッジ（段階3・2026-07-16 `5216ea2`）。カテゴリ機能は一通り稼働。
- ⏸ **保留（小野寺判断）**：縦長の畳み（ぶら下がり価格の折りたたみ）。段階3の絞り込みを実務で使い、**まだ冗長なら着手**。今は絞り込みで様子見。

### 宙に浮いている・要確認（着手前に事実確認）
- ❓ **マイページ（`orders.mypage_token`）の prod 反映状況を要確認**：過去に別チャットで「dev完成・prod未反映」のエラー履歴あり。着手・依頼の前に**本番DBの反映状態を確認**する（現状スナップショットでは本番稼働扱いだが、要現物確認）。

### 大きい工事（要設計・腰を据えて）
- ⬜ **合計請求書＋入金管理の作り直し**：現状は「**いつ**入金したか（`paid_at`）」は記録するが「**いくら**」を記録せず全額入金扱い。部分入金の記録・未入金の繰り越し・**入金テーブル新設**が必要。データモデルから設計し直す大工事。
- 🚧 **Stripe課金**：ブランチは**mainマージ済みと判明**（backup タグ `backup-before-stripe-merge` あり）。現状確認込みで着手する。残＝dev動作確認・liveキー・本番Webhook・本番price・billing金額調整（§2ワークストリーム参照）。

### 小さめ・寝かせ中
- ⬜ **社内品番をPDF/明細に出す**：`OrderItem.part_number`（フィールド追加）＋PDF列描画。二階建て化以降の継続残課題。
- ⬜ **PDFヘッダ繰り返しの罫線**：複数ページ時、2ページ目の見出し文字が出ない。寝かせ中。
- ⬜ **メニュー単機能化 段2**：旧カラムのDB DROP（現在は後方互換で温存中）。明細の列を金額1列に統合・「技術料」へ文言変更・PDF対応も含む。
- ⏸ **受注明細から新規部品登録**：保留。

> 凡例: ✅完了 ／ 🚧着手中・一部完了 ／ ⬜未着手 ／ ⏸保留 ／ ❓要確認
...
- 🚧 **Step 3**: 車種別定価（利益エンジン）。**Step 3-1（テーブル）・3-2a（variant登録UI）・3-2b（明細への⭐呼び出し＋定価反映）完了（2026-06-07・本番反映・本番で稼働確認済み）**。
  ...
  **設計方針（重要）**:
  ...
  - **焦って作らない領域**: データ構造（車種をどう持つか）を一度決めると変更コストが高い。
    ...
    - **【現在地・2026-07 追記】** 上記は当初方針。実際には二階建て（`parts_inventory_variants`）は
      Step 3（3-1/3-2a/3-2b）までアプリ化完了・**本番稼働済み（2026年6月）**。スプレッドシートに残しているのは
      「明細に化けない部分」＝測定値・作業手順・サス技術DB（②）のみで、部品＋車種別品番・定価はアプリに移行済み。
...
- ブランチ `feature/order-items-grid` に実装（main 未マージ・未デプロイ）。

**2026-09-24 追記（実機フィードバック対応・B案）**：数字セルが最初から "0" 表示で「4500 と打つと 45000」になる問題を修正。
新コンポーネント `GridNumberInput`（items-form.tsx 内）で、**PC幅(md以上)だけ**フォーカス時に値が "0" なら**表示だけ**空にし
（0以外は何もしない）、フォーカスを外すと表示を戻す。**親の state（保存データ）は「打った時（onChange）」しか変えない**設計＝
focus→blur で何も打たなければ保存内容は完全に不変（`labor_cost` に 0 が紛れ込む等もなし）。空欄のまま保存されても従来の
`Number()||0` 正規化で 0 保存（保存の中身・計算・合計・粗利は無改修）。対象＝**金額(unit_price)・数量・まとめ行の工賃/部品代/
...
> 現場の部品はほぼ「買う→すぐOHで使う」流れで棚に置かず、入庫登録もしないため在庫数はマイナスのまま。
> 発注点判定では全21件（活性）が常時「欠品／要発注」表示になっていた（実害なし・正常運用だがノイズ）。
> 一方で今後 EC で売る部品（FG Gubellini の補修部品）は売るために在庫を持ち数量を正確に保ちたい。
> この2種類を **`parts_inventory.track_stock`（真偽値）** で区別する。

**決定**
- **既定は「追跡しない」(`track_stock=false`)**。マイグレーションは `NOT NULL DEFAULT false` で ADD COLUMN し、
  **既存の全行（当時 23 行）も自動的に false で初期化**（＝全部品が従来の欠品表示から外れる）。EC 用に持つ
  部品だけ編集画面で ON にする運用。「大半は追跡しない」という現場実態に既定を合わせた。
- **追跡しない部品**：在庫数は表示するが、発注点判定・欠品/要発注バッジ・「発注が必要」アラート・
  ダッシュボードの要発注 KPI の対象から外す（一覧では中立の「追跡なし」グレーバッジ）。
- **追跡する部品**：従来どおり `stock_quantity <= reorder_point` で発注点判定・欠品表示。
- 一覧フィルタに **「追跡する部品のみ」** を追加。編集/新規フォームに **「在庫を追跡する」チェックbox** を追加。

**触っていないこと（制約の遵守）**
- **在庫RPC（reserve/release/consume/unconsume/deduct 系）・在庫数(`stock_quantity`)の計算ロジックは一切未改修**。
  変えたのは **表示上の扱い（バッジ・フィルタ・要発注カウント）だけ**。既存カラムの変更・削除もなし（追加のみ）。
- 二階建て(variants)・受注(orders)・PDF・棚卸/入庫にも非干渉。
- 定数デフォルトの ADD COLUMN は Postgres 11+ でメタデータのみの変更＝テーブル書き換えなしで安全。

**適用（DB先・コード後／prod は手動）— 本番反映済み**
- マイグレーション `supabase/migrations/20260910000000_add_track_stock_to_parts_inventory.sql` を作成。
- **prod MCP は読み取り専用のため、本番適用は小野寺が SQL Editor で手動実行済み**。事後確認結果＝
  `col_default=false / is_nullable=NO / total_rows=23 / untracked_rows=23 / tracked_rows=0`（＝既存全行が
  track_stock=false で初期化・期待どおり）。`supabase_migrations.schema_migrations` に `20260910000000` も登録済み。
- dev は解約済みのため適用先は prod のみ。
- **画面の動作確認はデプロイ後に実施予定（この commit 時点では未検証）**。追跡ON/OFFの切替・「追跡なし」バッジ・
  「追跡する部品のみ」フィルタ・要発注アラート/KPI/サイドバー数の除外を、デプロイ後に目視確認する。

### 2026-09-09 ── 部品在庫の商品画像（親側・最大5枚・任意・非公開バケット）・**prod DB適用済み／実機動作は未検証**

> 部品在庫に商品写真を持たせ、一覧・編集画面でサムネイル表示できるようにした。
> 将来ECサイト側から同じ画像を参照する前提で設計している。
> **prod にのみ適用（dev は Supabase 解約済みのため適用先が無い＝以降 dev 検証という選択肢は取れない）。**
> **DBの構造は確認クエリでグリーン。ただし実機の画面動作はコード未デプロイのため未検証**（下記「検証」参照）。

**【判断①】画像は親 `parts_inventory` に持たせる（`parts_inventory_variants` には持たせない）**
- 画像は「**物理部品そのものの写真**」であり、車種別の呼称＝売り方(variant)ごとには変わらない。
  同じスライドメタルの写真が、TZR用の品番と グース用の品番で別物になることはない。
- §3 の到達点「**部品は寸法で1行（在庫は割らない）／売り方だけを二階にぶら下げる**」と一貫する。
  一階＝物理的実体（原価・在庫・仕入れ品番・カテゴリ・**画像**）、二階＝売り方（社内品番・定価・掛率・車種）。
  §3 と矛盾しないことを実装前に確認済み。
- 副次効果: variant を増やしても画像を重複登録しなくて済む（枚数上限も部品単位で素直に数えられる）。

**【判断②】1部品あたり最大5枚・0枚も許容（画像は任意）**
- 部品マスターは「仕事のたびにその都度追加」（§3）なので、**画像必須にすると登録が止まる**。任意。
- 上限5枚は **DB側でも担保**した（アプリのバリデーションだけに頼らない）:
  - `CHECK (display_order >= 0 AND display_order < 5)` … 取り得る並び位置を5種類に限定。
  - BEFORE INSERT/UPDATE トリガ `parts_inventory_images_guard()` … `count(*) >= 5` で弾く。
  - **同時INSERTのすり抜け対策に `pg_advisory_xact_lock(part_id)` を使い、`parts_inventory` の
    行ロック（`SELECT … FOR UPDATE`）は意図的に使わない**。在庫RPC（reserve/release/consume/
    unconsume/deduct 系）が `parts_inventory` を UPDATE するため、**画像側からそこにロックを
    持ち込まない**＝在庫処理と原理的に競合させない、という線引き。在庫RPCは1行も触っていない。
- 同トリガで「画像の `user_id` は必ず親部品の所有者と一致」も見る。RLS の `user_id = auth.uid()`
  だけでは「自分の user_id で他テナントの part_id を指す行」を防げない（FKはテナントを見ない）ため。
  §2026-07-15 の `resolveCategoryId`（アプリ側で所有者検証）と同じ思想を、画像ではDB側にも置いた。

**【判断③】バケットは非公開（`part-images`, public=false）**
- **EC公開時に「原価と一緒に漏れる経路」を作らないため**。画像を公開URLで置くと、公開のために
  作った経路にそのまま社内情報（原価・仕入先）を通す作りへ引きずられやすい。
- 表示は**署名付きURLを都度発行**する。DBに持つのは `storage_path` だけで **URLは保存しない**
  （署名は失効するので保存する意味がなく、配信経路を後から差し替えられる）。
- 署名の発行口を `lib/parts/images-server.ts` の1か所に集約。**EC の非ログイン公開が必要になったら、
  service role で署名する専用ルートか公開用ミラーバケットを別に足す**（この関数は触らない）方針。
- ポリシーは既存 `shop-assets` と**完全に同じ形**に揃えた（先頭フォルダ = `auth.uid()` の4本）。
  パス形式 `<user_id>/<part_id>/<uuid>.<ext>`。MIME は jpeg/webp/png、上限3MB。

**【判断④】画像は「更新する」の一括保存に乗せず、操作した時点で即確定**
- 本体＋価格カードは単一 submit で一括保存（2026-06-24の流儀）だが、画像は実体を **Storage へ
  直接アップロード**するためフォームの submit と足並みを揃えられない。設定画面のロゴ/印鑑
  （`image-upload.tsx`）と同じ「アップロード → server action で登録 → `router.refresh()`」に揃えた。
- そのため画像セクションは**本体フォームの外**（独立カード）に置き、画面にも「選択・削除した時点で
  保存されます」と明記した。**新規登録画面には出さない**（part_id が無いため）。登録 → 編集画面で追加。
- 整合性: アップロード成功 → DB登録失敗 のとき、server action が**アップロード済み実体を消す**
  （Storageに迷子を残さない）。逆にDB削除成功 → 実体削除失敗は致命傷にせず、迷子を残すだけに留める。

**変更ファイル**
- DB（prod適用済み）: `supabase/migrations/20260909000000_create_parts_inventory_images.sql`（テーブル＋RLS4本＋
  ガードトリガ＋index3本）、`20260909010000_create_part_images_bucket.sql`（バケット＋storage.objects ポリシー4本）。
- 型: `lib/types.ts`（`PartImage` / `PartImageWithUrl`）。
- 共通: `lib/parts/images.ts`（定数・パス組立/検証。client/server 共用）、`lib/parts/images-server.ts`
  （署名付きURL発行の唯一の入口・`loadPartImages`・`loadPrimaryImageUrls`）。
- 画面: `app/dashboard/parts-inventory/image-actions.ts`（新規・register/delete/reorder）、
  `part-images-section.tsx`（新規・D&D並べ替え＋個別削除＋クライアント側リサイズ）、
  `[id]/edit/page.tsx`（画像セクション追加）、`page.tsx`＋`parts-inventory-table.tsx`（一覧に「画像」列）。
- **`actions.ts`（本体・価格カード・在庫）は無改修。`parts_inventory` の既存カラムも変更・追加なし。**

**クライアント側リサイズ（原寸のまま上げない）**
- 長辺 1600px に収めて再エンコード（WebP優先 → JPEG → PNG フォールバック、品質0.82）。
  `createImageBitmap(..., {imageOrientation:"from-image"})` で EXIF の向きを反映し、
  再エンコードの副次効果で **EXIF（撮影場所等）も落ちる**。JPEGフォールバック時に透過が黒く
  ならないよう、描画前に白で塗る。

**一覧のサムネイル**
- 各部品の代表画像＝`display_order` 最小の1枚。テナント分を1クエリで取り、`createSignedUrls`（複数版）で
  **まとめて署名**（部品数ぶんの往復にしない）。画像が無い部品は**プレースホルダー（枠だけの箱）**を出し、
  行の高さがガタつかないようにする。列は「並び」の右に幅16の「画像」列を追加（ぶら下がり価格行の colSpan を 5→6 に追随）。

**検証**
- `tsc --noEmit`・`next build`・`eslint`（変更ファイル）通過。
- **prod DB適用（2026-09-09・SQL Editor で2本とも実行）＋台帳登録（`20260909000000`/`20260909010000`）済み。**
  バケットも Dashboard ではなく `20260909010000` の SQL で作成した。
- prod の**構造**を確認クエリでグリーン確認：`parts_inventory_images` 存在／RLS 有効＋ポリシー4本／
  ガードトリガ1本 enabled='O'／`CHECK (display_order >= 0 AND display_order < 5)`／FK 2本とも ON DELETE CASCADE／
  `storage.buckets` に `part-images`（`public=false`）／`storage.objects` の `part_images_*` ポリシー4本／既存行 0 件。
- **⚠️ 実機（画面）動作はすべて未検証。** 確認できているのは上記の DB 構造だけで、
  **アップロード・D&D並べ替え・個別削除・5枚上限で止まること・一覧のサムネイル・画像なしのプレースホルダー・
  他テナントから見えないこと は、いずれも一度も動かしていない**。コードのデプロイ後に本番で必ず1周確認する。
- **dev での事前検証はできない**：dev の Supabase は解約済み（本件着手時点で MCP も権限エラーで接続不可だった）。
  以降このリポジトリの DB 変更は「prod に直接適用 → 本番で確認」しか経路が無い点に留意する
  （＝加算的・後方互換で、既存機能に触れない変更に限る運用がこれまで以上に重要）。
...
> （部品/variants/メニュー/セット/set_items/set_parts/カテゴリ/間接材＝**8本**）まで毎回取り直してブロックしていた」こと。
> 調査で「連動表示（バッジ・一覧カンバンの列移動・在庫バナー・帳票メタ）はすべて**受注行＋在庫フラグ由来で、重いカタログは
> 連動表示に不要**」と確定済みなので、カタログを Suspense に追い出しても連動表示は正しく出る。
> **最優先＝在庫の確実さ。在庫ロジックは一切触らない。楽観的更新もしない。** commit `27e5a26`（main ff・push済）。DB変更なし。

**シェル／カタログの分離（Suspense）**
- 受注詳細を「①受注行由来の軽いシェル」と「②明細ピッカー用カタログ（Suspense）」に分割：
  - **シェル（`page.tsx`）**：ステータスバー・顧客/車両・メモ・帳票出力・マイページ・明細の枠。取得は**軽いものだけ**
    （顧客・車両・Drive連携・マイページ可否・baseUrl）。カタログを待たない。`notFound()` は Suspense 前に置き 404 を維持。
  - **カタログ（新規 `order-items-section.tsx`）**：async サーバーコンポーネントが8本を取得し `ItemsForm` に供給。
    `<Suspense fallback=スケルトン>` で包む。ピッカーの DB フィルタ・並び・原価マップ導出は従来ロジックをそのまま移設。
- **`ItemsForm` 内部・保存・明細計算（calculateTotals）・PDF・作業セット・カテゴリ・D&D・在庫バナー
  （reserved_at/consumed_at）は一切変更していない。** カタログの「供給経路」だけを Suspense 子へ移動。

**在庫の確実さ（最優先事項）を1ミリも変えていない**
- ステータス変更の server action・`revalidatePath`・在庫RPC（reserve/consume/release/unconsume）は**未変更**。
- 楽観的更新はしない。**サーバーが在庫RPCを確実に実行し、その確定結果を待って表示する今の挙動を完全維持**。
  在庫RPC失敗時の status ロールバックも従来どおり。在庫数・予約数・在庫バナーの表示/計算は不変。

**なぜ連動表示が破綻しないか**
- サーバーが真実の源のまま。ステータス変更後の再レンダーで、**シェル（ステータスバー等）はカタログを待たずに更新**され、
  カタログは非ブロッキングで後追い（server action は transition なので Suspense 境界は再レンダー中も直前の内容を保持＝
  ItemsForm はフォールバックに戻らず in-place 更新、ローカル明細編集も保持）。連動表示（バッジ・カンバン列移動・在庫バナー・
  帳票メタ）は**受注行由来なので自動で正しい**。クライアントに業務ロジック（在庫・請求）を複製しない＝ちぐはぐが原理的に起きない。

**見込み・判断・残注意**
- 見込み：ステータス変更後の待ち時間から**カタログ8本の取得がブロック要因として外れる**。開くときもシェルが先に描画され体感改善。
- 判断：在庫バナーは `ItemsForm` 内にあるため Suspense 内側に残す（シェルへ抽出すると ItemsForm 改変になる）。バナーはカタログ
  再解決後に更新＝バッジよりわずかに遅れるが**常にサーバー確定値で正しい**。速さより確実さを優先した割り切り。
- **本番反映後、在庫関連（見積=了承済で確保／作業=完了で消費／取消で復元／RPC失敗でロールバック）が従来どおりであることを
  小野寺氏が実物で必ず確認**してから次へ（速さより確実さ）。tsc/build/eslint 通過。
...
> カテゴリで絞り込めるようにする。左に読み取り専用カテゴリツリー（件数バッジ付き）＋右に該当部品一覧。
> 目的＝「部品を探しやすく・登録済みか一目で分かる」。**DB変更なし（表示のみ・ノーマイグレ）**。
> commit `5216ea2`（main ff・push済）。新ファイル `category-filter-tree.tsx`。

**子孫解決＝方式②（クライアント木展開・調査で決定）**
- 上位カテゴリ（大・中）で絞ると配下の子孫すべての部品を集める必要がある。方式は①再帰CTE(DB) と
  ②クライアント木展開 の2案。**②を採用**。理由：
  - **ノーマイグレで済む**。①は supabase-js から `WITH RECURSIVE` を直接投げられず、RPC関数（DBオブジェクト）
    新設＝マイグレが必要で「表示のみ」方針に反する。②はDB変更ゼロ。
  - `part_categories` は左ツリー描画のため結局全件フェッチするので、子孫id集合の算出はメモリ上で無料。
  - 規模が小さい（数十件）ので性能面で①の優位が効かない。既存の `childrenOf` マップ展開
    （`category-select.tsx`／`part-categories-manager.tsx`）と**同型**で保守者が読める。
- 実装：`page.tsx` が `part_categories` を `level→sort_order` 順で追加フェッチ→テーブルへ渡す。テーブルは
  `childrenOf` を組み、選択ノードの**子孫id集合（自ノード含む）**を算出、既存 `filtered`(useMemo) に述語を1本追加。

**選択状態＝URL `?category=<id>`（include_deleted と同じ流儀）**
- 無指定=すべて表示 / `none`=未分類(`category_id IS NULL`) / それ以外=カテゴリid。`page.tsx` が
  `searchParams.category` を読んで `selectedCategory` を渡す。ツリーは `router.push` でURL更新（サーバー再
  フェッチ目的ではなく**選択の永続化**＝リロードで保つ）。絞り込み自体はクライアント側。

**件数バッジ＝子孫含む件数**
- 各カテゴリに配下合計を表示（大分類に総数が出る＝「登録済みか一目で分かる」に直結）。`countDescendants` と
  同型の後順走査で subtree 合計を算出。**母集合は表示中の行セット（`rows`＝include_deleted 状態を尊重）**で、
  **検索・発注フィルタには連動させない**（登録件数の意味を保ち、クリックでちらつかせない）。左件数と右一覧はズレない。

**既存挙動との共存（無改造）**
- **カテゴリ選択も `isFiltering` 扱いにした**だけで、既存の「フィルタ中はDnD無効」「件数テキスト(登録件数↔N件中M件)」
  判定が自動一致。検索(名前・品番・仕入先)・発注フィルタ・include_deleted・DnD並べ替え・**二階建て価格・在庫・
  各モーダル・列構成は一切変更なし**。カテゴリは左ツリーと述語1本を足すだけ。
- **カテゴリ未登録テナントは左ツリーを描画せず**（`categories.length===0`）、従来どおり右一覧のみに縮退＝破壊なし。
- レイアウトは外側を左右 flex に（左=`category-filter-tree`・幅固定/独自スクロール、右=既存テーブルほぼそのまま）。

**後方互換・無影響**
- 既存部品は `category_id=NULL`＝「未分類」に集まり従来どおり表示。段階2の SET NULL によりダングリング参照なし
  （削除カテゴリを指す部品は未分類に落ちるだけ）。段階2で先行付与の部分index を利用（新規index不要）。
- **作業カテゴリ(work_item_categories)には触れていない。** 縦長の畳み（ぶら下がり価格の折りたたみ等）は今回スコープ外。

**検証**
- `tsc --noEmit`・`next build` 成功・eslint クリーン。実物（左ツリー右一覧の使い勝手・件数・URL永続・既存機能の
  非破壊）は**小野寺氏が本番で確認**。
...
> データ構造＋管理画面まで**。部品への紐付け＝段階2、一覧の絞り込み＋縦長解消＝段階3（未着手）。
> commit `b9c7a6e`（main ff）。DB=`20260715000000_create_part_categories.sql`。

**背景（なぜやるか）**
- 部品在庫一覧が縦に長く探しづらい。主因は「1部品＝2行（本体＋全幅のぶら下がり価格行）」構造で、
  車種別variantが多いほど無限に伸びること。**カテゴリで絞れば表示件数が減る**のが最大の狙い（縦長解消は段階3）。

**設計（確定した姿）**
- **専用テーブル `part_categories` を新設**（作業カテゴリ `work_item_categories` とは**物理的に別・完全独立**）。
  ※構造は結果的に「自己参照ツリー」で既存と似るが、要件に対する正しい選択が一致しただけ。混在させない。
- **自己参照ツリー**：`parent_id`（大＝NULL）、`level`（1=大/2=中/3=小）、`sort_order`。
  - `CHECK(level BETWEEN 1 AND 3)` で**4階層目をDB拒否**。
  - `CHECK((level=1かつ親なし) OR (level>1かつ親あり))` で「親なしの中」等の不整合をDB拒否。
  - テナントは**1〜3階層を自由に**（中・小は任意）。分類の軸（部位/メーカー/種類）も自由。**デフォルトseedなし**。
- **RLS 4本**（`user_id=auth.uid()`・`TO authenticated`）＋ **DML GRANT を authenticated/service_role に明示**
  （直近 `work_menu_set_parts` の最新流儀。prodのdefault privileges非依存で **42501 を確実回避**）。
- **level はサーバーが親から算出**（`actions.ts`）。クライアント申告を信用せず `level<=3` をサーバーで弾き、
  DBの`CHECK`が二重に守る。UIも小分類(level3)には子追加ボタンを出さない（三重ガード）。

**判断した点（想定外・申し送り）**
- **「既存 categories / `/dashboard/categories`」は実在しなかった**。実体は `work_item_categories`
  （サイドバー「カテゴリ管理」）。それを設計の手本にした。
- **削除＝ハード削除＋`ON DELETE CASCADE`**（`deleted_at` なし）を採用。理由：段階1は部品参照が無く木が
  自己完結／仕様のカラム列挙に`deleted_at`が無い／木のソフト削除（親だけ非表示で子が浮く）を回避。
  **→ 段階2で `parts_inventory.category_id` を足す際は、部品側を `ON DELETE SET NULL` 等で守ること**（重要な申し送り）。
- 管理導線は**新ルート `/dashboard/part-categories`＋サイドバー「部品カテゴリ」を新設**。既存「カテゴリ管理」
  （作業用）とは別行で並存させ、混同を避けた。

**検証（dev実データ）**
- 3階層作成OK／4階層目は`check_violation`で拒否／親なしlevel2も拒否／大削除で中・小がCASCADEで消える／残存0件。
- RLS有効・4ポリシー・CHECK2本・GRANT付与を確認。`tsc --noEmit`・`next build` 成功。
- **prod適用済み**（SQL Editor手動・事前0/1・本体Success・事後 rls=true/policy=4/check=2/grant付与 を小野寺氏が確認）。

**無影響**
- `work_item_categories`／`/dashboard/work-item-categories`／`orders.items` 系には一切触れず。新テーブル・
  新ルート・サイドバー1行の**加算のみ**。

**次段階**
- 段階2＝部品への紐付け（`parts_inventory.category_id` 追加・登録/編集画面でカテゴリ選択）。
- 段階3＝一覧のカテゴリ絞り込み（左ツリー＋右一覧）＋縦長解消（ぶら下がり畳み等）。

### 2026-07-10 ── 受注明細の作り直し（全体・設計と段階分けの総括）

> 個別の実装詳細は後続の「段階1」「段階2」「段階3」エントリ参照。ここは**なぜ作り直したか・
> 何が分かって・どう分割したか**の総括。段階1〜3すべて本番反映済み（DB変更なし）。

**背景（作り直しの動機）**
- 受注明細が「**作業内容＋部品名＋金額1つ**」を1行に同居させる構造で、作業と部品が混ざっていた。
  一方、マスター側は「作業メニュー」と「部品在庫（二階建て価格解決＝車種一致→汎用・個人定価/業者業販）」に
  分かれて進化しており、**明細の持ち方がマスターの構造と乖離**していた。手入力のしやすさも不足していた。

**設計（確定した姿）**
- **手入力を主役**に。**種別（作業/部品）**を選ぶ簡易入力＋**「詳細」展開（freee式）**。
- 金額に**業販/定価バッジ**（受注の顧客区分で明示）。
- **1行まとめ**＝作業1＋部品1を1行表示（**作業名と部品名を両方表示**・工賃/部品代/小計＝他店式）。
- **原価・利益は内部のみ**（お客様帳票には出さない）。

**重要な発見（＝全面作り直しを回避できた理由）**
- 事前調査で、新設計の**約9割は既存実装で実現済み**と判明：行の種別判定（work_name/part_name/linked_part_id）、
  部品在庫の二階建て価格解決（`effectiveVariantByPart`/`rowFromPart`）、作業セット保存/展開、在庫連携。
- さらに**本番実データがクリーン**（9受注137行・作業と部品の同居は実質1行のみ・間接材料0）と確認。
- 結論：**心臓部の全面作り直しは不要**。「手入力まわりの整理（段階1・2）＋新機能2つ（バッジ・1行まとめ）」で完結し、
  **既存明細データの移行も不要（すべて後方互換）**にできた。

**段階分け（すべて本番反映済み・各DB変更なし）**
- **段階1**：業販/定価バッジ。判定は**受注の顧客区分に統一**（business=業販/personal・未設定=定価）。表示のみ。
- **段階2**：手入力の簡易入力（種別・名前・数量・金額）＋**「詳細」展開**（原価/定価/業販/行のマスター登録）。
  税区分は詳細に出さない（既存税ロジックは維持）。種別セレクタは手入力行のみ（マスター由来は固定）。
- **段階3**：1行まとめ。方針は**【表示結合】**＝データは作業行・部品行を**別々のまま保持**し、`parent_uid` で
  グループ化、**表示・PDFのときだけ1行結合**。作業1＋部品1に制限。

**段階3の効果（表示結合を選んだ理由）**
- データが別行のままなので **`calculateTotals`/`calculateProfit`/`calculateListPriceTotals`・在庫連携
  （reserve/consume）・原価利益は完全無改修**。
- **既存明細は `parent_uid` 無し＝まとめOFF**で従来どおり別行表示＝**無損失・後方互換**。
- **DB変更なし**（`parent_uid`/`uid` は `orders.items` jsonb 内で完結・DDL不要）。

**検証**
- 見積書PDF（`26MB-0002`）で、結合行「**オーバーホール＋オイルシール 工賃¥10,000／部品代¥1,000＝¥11,000**」が
  **1行表示**、他行は別行、**複数ページで崩れず**、**合計に正しく反映**されることを確認。

**残確認（次回）**
- 段階3の**画面操作**：部品2つ目の制限・まとめ解除・金額不変・（自店式別行の）在庫連携・既存受注の表示/PDF の最終確認。
...
**位置づけ**
- 明細作り直しの **段階2**（段階1＝業販/定価バッジ完了の次）。手入力を主役にした入力体験へUI再構成。**DB構造・保存データは不変**。

**決めたこと（本番反映・`items-form.tsx` 1ファイル・DBマイグレなし）**
- **簡易入力行を [# / 種別 / 名前 / 数量 / 金額＋バッジ / 詳細・×] に再構成**。旧2段グリッド（作業内容＋部品名の2入力＋原価/業販/参考定価/小計の下段）を廃止し、**種別トグル（作業/部品）＋単一「名前」欄**にした。金額欄には段階1の業販/定価バッジを継続表示。行の小計は直下に控えめ表示。
- **種別（kind: labor/part）はUI専用・非保存**。`ItemRow` に `kind` を追加したが `toItem` では書かず、保存は従来どおり **work_name / part_name / linked_part_id** で表現。`toRow` が既存フィールドから種別を推定（work_name空かつ部品名/在庫リンク有り→部品、他→作業＝段階1の分類ルールと同一）するため、**保存→再読込で種別が自然に復元**。名前欄は種別に応じて work_name か part_name を編集し、種別切替時は入力済みの名前を移送してもう片方を空にする。
- **マスター由来の行（linked_part_id / source_menu_id 有り）は種別トグルを無効化**（在庫・メニュー連携を勝手に壊さないため）。
- **「詳細」パネル（行ごとに折りたたみ）** に freee 式で **原価（社内・帳票非表示）／定価（list_price、今回から行編集可能に）／業販（法人=金額×数量の確認表示・個人=定価である旨）／「☆ この行をマスター登録」（既存 openMenuModal を流用）／補足（＋補足を内包）** を格納。**税区分は詳細に出さない**（既存税区分ロジックは `toItem`/`parseItems` で従来どおり維持）。補足に既存文言がある行は詳細を自動展開して中身を隠さない。
- **「行のセット化ボタン」= 既存の ☆ 行単位マスター登録を詳細に移設**と解釈（受注全体のセット化 `SaveAsSetButton`＝`saveOrderAsSet` は別導線として不変）。単一行→新規セット生成の新サーバー処理は段階2では作らない（スコープ最小・無リスク）。
- **完全にUI再構成のみ**：`calculateTotals/Profit/ListPrice`、`parseItems`/`toItem` の保存形、PDF、在庫RPC、作業セット、D&D（`_uid` キー据置）に手を入れていない。金額は従来どおり `unit_price`（更新は既存 `updateUnitPrice`＝labor_cost へ書き unit_price 同期）。
- **スコープ/検証**：新ブランチ `feature/meisai-stage2-simple-detail`。`tsc --noEmit`／`eslint`（対象ファイル）／`next build`（✓ Compiled・34/34 生成）通過。dev確認は挟まず本番で小野寺氏確認。
- **次段階**：段階3＝「1行まとめ」（作業に部品1つを付随して1行表示・PDFも1行結合）。方針は **parent_uid のグルーピング＝表示結合で計算/在庫は無改修**。ここで初めて新フィールド（parent_uid 等）を扱う想定。

### 2026-07-11 ── 明細作り直し 段階1：単価に「業販/定価」バッジ（表示のみ・本番反映）

**位置づけ**
- 「受注明細の作り直し」の段階分けのうち **段階1**。作り直しは調査→計画済み（要点は後述）。今回はその最初の
  実装ステップ＝「今その行に入っている金額が業販価格なのか定価なのかを、明細上で一目で分かるようにする」だけ。

**決めたこと（本番反映）**
- 受注明細フォーム（`app/dashboard/orders/[id]/items-form.tsx`）の各行**単価欄のラベル横**に、小さなバッジ
  `PriceKindBadge` を出す。**業者受注（`customer_type==='business'`）＝「業販」（オレンジ）／個人・未設定＝「定価」（グレー）**。
- **判定基準は受注の顧客区分のみに統一**。理由：部品行は二階建て価格解決で business=業販価格／personal=定価が既に
  `unit_price` に入っており、作業行も `rowFromMenu` で business かつ `markup_rate` 有りなら業販価格が入る（無ければ定価のまま）。
  行ごとの掛け率有無でバッジを出し分けると「業者なのに定価表示」が混ざって直感に反するため、**受注単位で業販/定価を明示**する
  既存の業販UI（法人=単価/業販/参考定価の3列化）と足並みを揃えた。
- **完全に表示専用**：金額の値・計算（`calculateTotals/Profit/ListPrice`）・保存（`parseItems`/`toItem`）・PDF・
  在庫連携・作業セットには一切触れていない（バッジ用 `<span>` を足し、単価ラベルを `block`→`flex` に変えただけ）。
- **スコープ**：`items-form.tsx` 1ファイル。DBマイグレなし。新ブランチ `feature/meisai-stage1-price-badge` で実装し
  typecheck（`tsc --noEmit`）／build（`next build` ✓ Compiled）通過を確認して main にff。dev確認は挟まず本番で小野寺氏確認
  （DB変更なしの軽微な表示変更のため）。
- **次段階**：段階2＝手入力を主役にした簡易入力＋「詳細」展開（freee式：原価/定価/業販・行セット化ボタン）のUI再構成。
  さらに先に「1行まとめ（作業に部品1つ付随）」を **parent_uid のグルーピング＝表示結合で、計算/在庫は無改修** に閉じる方針
  （本番実データは9受注137行・両方持ちは実質1行のみ・間接材料0でクリーン、移行は後方互換読み取りで無損失にできる、と確認済み）。

### 2026-07-08 ── マイページ 見積セクションの条件付き折りたたみ（本番反映）

**背景・問題**
- マイページで「お見積り」セクションと「作業完了」セクションが**同じ `items` から同じ明細を描画**しており、
  請求フェーズでは同一内容が重複して見えて冗長だった（見積と請求の明細が「二重に見える」問題）。
- お客様に見せたい情報は請求フェーズでは「作業完了カード（＝確定内容）」＋「お支払い案内」であって、
  見積は**参考**の位置づけになる。見積を消すと情報が失われるので「隠すだけ・クリックで復元可能」にしたい。

**決めたこと（本番反映）**
- **折りたたみ条件は AND**：`(invoice_status === "請求済" || "入金済") && work_status === "完了"` のときだけ
  見積セクションをネイティブ `<details>/<summary>` で折りたたむ（デフォルト閉じ）。タイトルは
  **「（参考）お見積り内容 ▼」**、accent枠を外して控えめなトーン＝「積極的に見せる情報ではない」ことを暗示。
- **★AND にした理由（重要）★**：`請求済` 単独条件だと、「請求済だが作業未完了」（前払い・部品先取り等の運用）で
  作業完了カードが存在せず、見積を畳むと**展開画面から明細が完全に消える**。InvoiceSection（請求済）は金額サマリのみで
  明細を持たないため、明細の担い手は「作業完了カード」だけ。そこで **作業完了カードが明細を担っている状態のときに
  限って**見積を折りたたむ。未完了のケースは従来通り展開したまま残す。
- **未請求のうちは従来通り**：`invoice_status === "未請求"` は展開表示（`<Card title="お見積り" accent>`）を維持。
- **折りたたみ時も導入文（「ご承認ありがとうございます」等）は中身に保持**：開いたとき文脈が読める。
- **サーバーコンポーネントのまま実装**：ネイティブ `<details>` を使い `"use client"` を追加しない
  （クライアント境界を増やさない・6/23フェーズAの「フェーズごとに表示が変わる」設計思想を継承）。
  標準の三角マーカーは `list-none` ＋ webkit マーカー非表示で消し、`▼` を `group-open:rotate-180` で回転。
- **スコープ**：コードのみ・DB変更なし（`load.ts` / `phase.ts` / RPC `mypage_get_by_token` は不変）。
  変更ファイルは `app/mypage/[token]/mypage-view.tsx` の `EstimateSection` 1ヶ所のみ。
- **dev検証**：トークン有効な `26MB-0002` を一時的にステータス切替して同一URLで4状態を目視確認
  （①入金済+完了=折りたたみ ②未請求+完了=展開 ③請求済+作業中=展開＝AND ガード動作 ④請求済+完了=折りたたみ）→
  確認後に元（発行済/完了/入金済）へ復元。typecheck/lint（対象ファイル）/build 通過。
- commit `26ea874`（mypage-view.tsx）・Vercel自動デプロイ。
...
- **2026-07-08** ｜ マイページ 見積セクション折りたたみ（本番反映） ｜ 見積と請求で同一 `items` を二重描画していた重複を、`(invoice_status∈{請求済,入金済}) && (work_status=完了)` のときだけ見積セクションをネイティブ `<details>` で折りたたむことで解消（「（参考）お見積り内容 ▼」・デフォルト閉じ・accent枠なし）。**AND条件**の狙いは「請求済だが作業未完了（前払い運用）」で明細が展開画面から消えるのを防ぐこと＝作業完了カードが明細を担う状態のときだけ畳む。折りたたみ時も導入文は保持。`"use client"` を足さずサーバーコンポーネントのまま（`list-none`＋webkitマーカー非表示＋`group-open:rotate-180`）。`load.ts`/`phase.ts`/RPC不変・DB変更なし・`mypage-view.tsx` の `EstimateSection` 1ヶ所のみ。dev検証＝`26MB-0002` を一時的に4状態へ切替し同一URLで目視確認→元（発行済/完了/入金済）へ復元。typecheck/build通過 ｜ commit `26ea874`（mypage-view.tsx）・Vercel自動デプロイ＋DECISIONS.md
- **2026-07-10** ｜ 明細作り直し ｜ 受注明細の作り直しを段階1〜3で本番反映。事前調査で「新設計の約9割は既存実装（種別判定・部品在庫の二階建て価格解決・作業セット）で実現済み・本番データもクリーン（同居行は実質1行）」と判明し、全面作り直しを回避＝手入力整理＋新機能2つで完結・既存データ移行不要。**段階1**=業販/定価バッジ（受注の顧客区分で判定・表示のみ）｜commit `00d942d`。**段階2**=手入力の簡易入力（種別/名前/数量/金額）＋「詳細」展開（原価/定価/業販/行マスター登録・税区分は出さない）・種別はUI専用で保存は従来フィールド｜commit `ebf73c2`。**段階3**=1行まとめ＝【表示結合】（作業行・部品行は別々のまま保持し `parent_uid` でグループ化・表示/PDFのみ1行結合・作業名と部品名を両方表示・作業1+部品1）。計算/在庫/原価利益は無改修（別行のため）・既存明細は parent_uid 無しで無損失・DB変更なし（jsonb内完結）・見積PDF `26MB-0002` で結合行「オーバーホール＋オイルシール 工賃¥10,000/部品代¥1,000=¥11,000」の1行表示と合計反映を確認｜commit `102cd93`。全段階DB変更なし・typecheck/eslint/build通過 ｜ commit `00d942d`/`ebf73c2`/`102cd93`
- （以降追記）
...
- **【検討済み・対応不要・2026-06-23】管理番号／品番のテナント間重複対策**：
  ...
  - **将来の発展アイデア（重複対策ではなく利便性向上・今やる話ではない）**：
    ...
    必要になったときに検討する。
- **【一部対応済・2026-06-23／2026-07-08】マイページ表示の検討（何を見せる/見せないか）**：マイページの「作業状況」が明細の
  `work_name` を全件再掲して冗長だった件は、**フェーズAで解決**＝進捗フェーズバーに置換し作業中の明細再掲を廃止
  （§4 2026-06-23参照）。さらに**見積と請求（作業完了カード）の明細が二重に見える件を 2026-07-08 に解消**＝
  請求済/入金済かつ作業完了のとき見積セクションを `<details>` で「（参考）お見積り内容」に折りたたむ（§4 2026-07-08参照）。
  残テーマ：明細をマイページに出すか受注ごと/設定で選べるようにするか／差分表示（見積↔請求の変更点強調）は引き続き将来検討。
...
## 作業セット一覧で部品を汎用価格で参考表示（表示のみ・金額固定なし）（2026-07-06）

**趣旨**：作業セット一覧（work-menu-sets）はメニューしか表示・集計しておらず、`work_menu_set_parts` に入っている部品が出なかった。部品を「汎用価格（車種指定なしの標準価格）での参考金額」として表示する。金額は固定保存せず、表示時に算出。受注展開の車種別解決は一切変更しない。

**DB変更**：なし。`work_menu_set_parts` は part_id＋quantity のまま。表示側だけの対応。

**参考価格の基準（判断）**：**汎用バリアント（vehicle_tags 空）の定価 `list_price`** を採用。無ければ部品の `sale_price`、それも無ければ「—」。
- 理由：セット一覧は特定受注（顧客）に紐づかず、業販掛け率は顧客別なので一貫適用できない。定価は普遍的なカタログ価格で、`rowFromPart` の個人向け（掛け率なし）＝「汎用の定価」と同基準。
- 汎用 variant の選び方は受注展開 `effectiveByPart` の汎用側（generalByPart）と同一（vehicle_tags 空・display_order 先頭の1件）。

**「参考」の明示**：部品セクションに「参考価格」バッジ＋注記「※ 部品は汎用価格（車種指定なし）の目安です。受注では車両に合わせた車種別価格で計算されます。」。ヘッダ合計も「部品(参考) N件 ¥○○」と表記し、確定金額と誤解させない。

**合計表示の形**：ヘッダを「メニュー N件 ¥42,100 ／ 部品(参考) M件 ¥○○」に変更（部品ゼロなら部品部分は非表示＝従来と同じ）。メニュー合計と部品参考合計は分離表示（合算しない）。

**実装**：page.tsx で work_menu_set_parts・parts_inventory(id,name,sale_price)・parts_inventory_variants を読み、part ごとに汎用 variant→参考単価を解決して rows.parts に載せる。work-menu-sets-list.tsx に部品セクション＋注記を追加。汎用価格が無い部品は行に「—」。
